        ...
        case .takeFullPicture:
            if let viewController = viewController {
                DispatchQueue.main.async {
                    ...
                    cameraVC.completionHandler = { base64Images in
                        // Handle the base64Images array directly here
                        self.successHandler(base64Images)
                        viewController.dismiss(animated: true, completion: nil)
                    }
                    viewController.present(cameraVC, animated: true, completion: {
                        // Ensure the camera is presented after the view controller transition has completed
//                        cameraVC.presentCamera()
                    })
                }
            }
        }
    ...
    /**
     ...
     @param controller      the ViewController that starts the document scan
     */
    private func goBackToPreviousView(_ controller: UIViewController?) {
        DispatchQueue.main.async {
